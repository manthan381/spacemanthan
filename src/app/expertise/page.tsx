    ...
    <main>
      ...
      <section className="relative isolate overflow-hidden bg-gray-100 pt-24 md:pt-32 pb-12 md:pb-16">
        <div className="absolute inset-0 opacity-[0.05]" aria-hidden="true">
          <div className="h-full w-full bg-[radial-gradient(circle_at_20%_20%,#000000_0%,transparent_50%),radial-gradient(circle_at_80%_0%,#4e6b5f_0%,transparent_50%)]" />
        </div>
        ...
        <div className="relative mx-auto max-w-[1600px] w-full px-6 sm:px-10 lg:px-24 grid grid-cols-1 md:grid-cols-2 gap-12 lg:gap-20 items-center">
          <div className="flex flex-col justify-center">
            <h1 className="text-3xl md:text-4xl lg:text-5xl font-bold leading-tight lowercase text-black">
              expertise
            </h1>
            <div className="h-1.5 w-20 bg-black mt-4 rounded-none" />
            ...
            <div className="mt-8 flex flex-wrap gap-3">
              <Link
                href="/projects?category=Office%20%26%20Residence"
                className="inline-flex items-center gap-2 rounded-none bg-black px-5 py-2.5 text-white hover:bg-black transition-colors duration-300 text-sm font-medium lowercase"
              >
                build
              ...
              <Link
                href="/services/furniture"
                className="inline-flex items-center gap-2 rounded-none border border-black px-5 py-2.5 text-black hover:bg-black hover:text-white transition-colors duration-300 text-sm font-medium lowercase"
              >
                furniture
                <ArrowRight className="h-4 w-4" />
              </Link>
            ...
            >
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-40 xl:gap-64">
                <div className="flex flex-col">
                  <h2 className="text-2xl md:text-3xl lg:text-4xl font-semibold text-black lowercase tracking-tight">
                    {service.title}
                  </h2>
                  ...
                  <div className="mt-8 lg:mt-12">
                    <Link
                      href={service.href}
                      className="inline-flex items-center gap-3 rounded-none bg-black px-7 py-3 text-white hover:bg-black transition-colors duration-300 lowercase text-sm font-medium"
                    >
                      {service.cta}
                      <ArrowRight className="h-4 w-4" />
                    </Link>
                ...
                <div className="flex flex-col justify-start">
                  <div className="space-y-6">
                    <p className="text-sm tracking-[0.25em] uppercase text-black font-semibold mb-4">
                      capabilities
                    </p>
                    <ul className="space-y-4 w-full">
                      {service.options.map((option) => (
                        <li
                          key={option}
                          className="flex items-center justify-between text-base sm:text-lg md:text-xl font-light text-gray-800 lowercase border-b border-gray-200 pb-4 hover:text-black transition-colors cursor-default"
                        >
                          {option}
